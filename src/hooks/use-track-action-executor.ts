	...
	const executeAction = useCallback(
		async (actionId: string) => {
			...
			switch (actionId) {
				...
				case CORE_ACTION_IDS.PLAY_NEXT_FROM_JIOSAAVN: {
					const result = await playbackService.appendRecommendationsFromTrack(
						currentTrack,
						5
					);
					if (result.success) {
						success(
							result.data > 0 ? 'JioSaavn next queued' : 'No suggestions found',
							currentTrack.title
						);
					} else {
						error('Failed to add JioSaavn suggestions', result.error.message);
					}
					return;
				}
